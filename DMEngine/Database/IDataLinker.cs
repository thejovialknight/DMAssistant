...
namespace DMEngine.Database
{
    public interface IDataLinker
    {
        ...
        T LinkObject<T>(string key, T value) where T : IDataLinkable, new(); // generics to pass as type rather than instance? maybe?
        List<T> LinkObjectList<T>(string key, List<T> value) where T : IDataLinkable, new(); // generics to pass as type rather than instance? maybe?
    }
}
